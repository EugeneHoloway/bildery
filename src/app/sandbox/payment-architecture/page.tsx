...
const layers: LayerDef[] = [
  {
    ...
    nodes: [
      ...
      { id: 'admin',    label: 'Admin Panel',       sub: 'PSP Management | Routing',     Icon: Settings,   phases: [4],   href: '/sandbox/payment-architecture/admin-panel' },
      { id: 'status',   label: 'Status Polling',    sub: 'Webhook events -- UI',         Icon: RefreshCw,  phases: [2],   href: '/sandbox/payment-architecture/status-polling' },
      { id: 'outbound', label: 'Outbound Webhooks', sub: 'Push events → Operator',       Icon: Send,       phases: ['next'],   href: '/sandbox/payment-architecture/outbound-webhooks' },
    ],
  },
  {
    id: 'api',
    ...
    nodes: [
      { id: 'auth',   label: 'Auth & Brand',    sub: 'JWT + Brand context',           Icon: Lock,         phases: [1],   href: '/sandbox/payment-architecture/auth-brand' },
      { id: 'rate',   label: 'Rate Limiting',   sub: 'Velocity checks',               Icon: Shield,       phases: [2],   href: '/sandbox/payment-architecture/rate-limiting' },
      { id: 'router', label: 'Request Router',  sub: 'Deposit | Withdrawal routes',   Icon: ArrowUpRight, phases: [2],   href: '/sandbox/payment-architecture/request-router' },
    ],
  },
  {
    id: 'orchestration',
    ...
    nodes: [
      { id: 'orchestrator',    label: 'Payment Orchestrator', sub: 'Routing rules engine',          Icon: Cpu,               wide: true, phases: [2],   href: '/sandbox/payment-architecture/orchestrator' },
      { id: 'cascade',         label: 'Cascade Manager',      sub: 'Failover PSP1 → PSP2 → PSP3',       Icon: Link2,                         phases: [2],   href: '/sandbox/payment-architecture/cascade-manager' },
      { id: 'statemachine',    label: 'State Machine',        sub: 'INITIATED → PROCESSING → DONE',     Icon: Repeat2,                       phases: [1],   href: '/sandbox/payment-architecture/state-machine' },
      { id: 'personalization', label: 'Personalization',      sub: 'BIN + preferred method',        Icon: User,                          phases: [5],   href: '/sandbox/payment-architecture/personalization' },
      { id: 'limits',          label: 'Limits & Rules',       sub: 'Min/max, velocity, KYC gate',   Icon: SlidersHorizontal,             phases: [3],   href: '/sandbox/payment-architecture/limits-rules' },
    ],
  },
  {
    id: 'adapter',
    ...
    nodes: [
      { id: 'interface', label: 'Unified Interface', sub: 'initiateDeposit | getStatus | handleWebhook', Icon: Plug,   wide: true, phases: [1],   href: '/sandbox/payment-architecture/unified-interface' },
      { id: 'adapter1',  label: 'PSP #1 Adapter',    sub: 'Your current provider',                      Icon: Zap,   accent: true, phases: [1], href: '/sandbox/payment-architecture/psp1-adapter' },
      { id: 'add-psp',   label: 'How to add a PSP',  sub: 'Checklist for new integrations',              Icon: Wrench,              phases: ['next'],  href: '/sandbox/payment-architecture/add-psp' },
    ],
  },
  {
    id: 'psp',
    ...
    accentTextCls:   'text-amber-500',
    nodes: [
      { id: 'psp1', label: 'PSP #1 API', sub: 'Cards | Crypto',  Icon: Globe, accent: true, phases: [1], href: '/sandbox/payment-architecture/psp1-api' },
    ],
  },
  {
    id: 'support',
    ...
    nodes: [
      { id: 'wallet',         label: 'Wallet Engine',        sub: 'Balance | ledger',            Icon: Wallet,       phases: [2], href: '/sandbox/payment-architecture/wallet-engine' },
      { id: 'kyc',            label: 'KYC | AML',            sub: 'Verification gates',          Icon: CheckCircle2, phases: [3], href: '/sandbox/payment-architecture/kyc-aml' },
      { id: 'bonus',          label: 'Bonus Engine',         sub: 'Wager | locks',               Icon: Gift,         phases: [3], href: '/sandbox/payment-architecture/bonus-engine' },
      { id: 'reconciliation', label: 'Reconciliation',       sub: 'Nightly PSP sync',            Icon: BarChart2,    phases: [2], href: '/sandbox/payment-architecture/reconciliation' },
      { id: 'chargeback',     label: 'Chargeback Management',sub: 'Disputes | evidence | rates', Icon: ShieldAlert,  phases: [4], href: '/sandbox/payment-architecture/chargeback' },
      { id: 'fx',             label: 'FX / Multi-currency',  sub: 'Rate lock | settlement vs display', Icon: Coins, phases: [3], href: '/sandbox/payment-architecture/fx-currency' },
    ],
  },
]
    ...
    >
      ...
      <DocSection num="3" title={lang === 'ua' ? 'Референсні документи' : 'Reference Documents'}>
        <div className="grid grid-cols-1 gap-3 tablet:grid-cols-2 desktop:grid-cols-3">
          {[
            { label: { en: 'Data Model',        ua: 'Модель даних'          }, sub: { en: 'DB tables: transactions, psp_configs…',    ua: 'Таблиці БД: transactions, psp_configs…'   }, href: '/sandbox/payment-architecture/data-model',        phase: '0' },
            { label: { en: 'State Machine',     ua: 'State Machine'         }, sub: { en: 'PassimPay statuses → UnifiedStatus',       ua: 'Статуси PassimPay → UnifiedStatus'         }, href: '/sandbox/payment-architecture/state-machine',     phase: '1' },
            { label: { en: 'Unified Interface', ua: 'Unified Interface'     }, sub: { en: 'IPaymentProvider types & API contract',    ua: 'Типи IPaymentProvider та API контракт'     }, href: '/sandbox/payment-architecture/unified-interface', phase: '1' },
            { label: { en: 'Auth & Brand Spec', ua: 'Auth & Brand Spec'     }, sub: { en: 'JWT validation & brand resolution',        ua: 'Валідація JWT та резолюція бренду'         }, href: '/sandbox/payment-architecture/auth-brand',        phase: '1' },
            { label: { en: 'PSP #1 Adapter',    ua: 'PSP #1 Адаптер'       }, sub: { en: 'PassimPay implementation spec',            ua: 'Специфікація реалізації PassimPay'         }, href: '/sandbox/payment-architecture/psp1-adapter',      phase: '1' },
            { label: { en: 'PSP #1 API',        ua: 'PSP #1 API'            }, sub: { en: 'PassimPay service overview & onboarding', ua: 'Огляд сервісу PassimPay та онбординг'      }, href: '/sandbox/payment-architecture/psp1-api',          phase: '1' },
            { label: { en: 'Status Polling',      ua: 'Status Polling'          }, sub: { en: 'How frontend tracks transaction status',           ua: 'Як фронтенд відстежує статус транзакції'                  }, href: '/sandbox/payment-architecture/status-polling',     phase: '2' },
            { label: { en: 'Outbound Webhooks',  ua: 'Outbound Webhooks'        }, sub: { en: 'Push events to operators -- retry, signing, delivery', ua: 'Push-події до операторів -- retry, підписання, доставка' }, href: '/sandbox/payment-architecture/outbound-webhooks',  phase: 'next' },
            { label: { en: 'Rate Limiting',     ua: 'Rate Limiting'         }, sub: { en: 'Velocity checks & abuse protection',       ua: 'Velocity checks та захист від зловживань'   }, href: '/sandbox/payment-architecture/rate-limiting',     phase: '2' },
            { label: { en: 'Request Router',      ua: 'Request Router'          }, sub: { en: 'Deposit & withdrawal endpoints + validation',  ua: 'Ендпоінти депозиту/виведення + валідація'    }, href: '/sandbox/payment-architecture/request-router',  phase: '2' },
            { label: { en: 'Payment Orchestrator', ua: 'Payment Orchestrator' }, sub: { en: 'PSP selection, routing rules, cascade logic',   ua: 'Вибір PSP, routing rules, логіка каскаду'    }, href: '/sandbox/payment-architecture/orchestrator',    phase: '2' },
            { label: { en: 'Cascade Manager',     ua: 'Cascade Manager'      }, sub: { en: 'Failover logic: PSP1 → PSP2 → PSP3',            ua: 'Failover логіка: PSP1 → PSP2 → PSP3'         }, href: '/sandbox/payment-architecture/cascade-manager', phase: '2' },
            { label: { en: 'Wallet Engine',       ua: 'Wallet Engine'        }, sub: { en: 'Balance: credit, debit, hold. Built on Orleans',  ua: 'Баланс: credit, debit, hold. Побудований на Orleans' }, href: '/sandbox/payment-architecture/wallet-engine',   phase: '2' },
            { label: { en: 'Reconciliation',      ua: 'Reconciliation'       }, sub: { en: 'Nightly PSP sync, discrepancy detection & fix',   ua: 'Нічна синхронізація PSP, виявлення розбіжностей'     }, href: '/sandbox/payment-architecture/reconciliation',  phase: '2' },
            { label: { en: 'FX / Multi-currency',  ua: 'FX / Мультивалютність' }, sub: { en: 'Rate lock, settlement vs display, historical rates',  ua: 'Фіксація курсу, settlement vs display, historical rates' }, href: '/sandbox/payment-architecture/fx-currency',    phase: '3' },
            { label: { en: 'Limits & Rules',       ua: 'Limits & Rules'        }, sub: { en: 'Min/max, caps, KYC gates, RG, AML -- rule engine',  ua: 'Min/max, ліміти, KYC-гейти, RG, AML -- движок правил' }, href: '/sandbox/payment-architecture/limits-rules',   phase: '3' },
            { label: { en: 'KYC | AML',           ua: 'KYC | AML'            }, sub: { en: 'Identity verification, sanctions, AML monitoring',   ua: 'Верифікація особи, санкції, AML моніторинг'          }, href: '/sandbox/payment-architecture/kyc-aml',         phase: '3' },
            { label: { en: 'Bonus Engine',        ua: 'Bonus Engine'         }, sub: { en: 'Deposit match, wagering tracking, balance lock, expiry', ua: 'Депозит-матч, відстеження вейджеру, лок балансу'    }, href: '/sandbox/payment-architecture/bonus-engine',    phase: '3' },
            { label: { en: 'Admin Panel',              ua: 'Адмін Панель'              }, sub: { en: 'PSP, routing, methods, partners, compliance — control plane',         ua: 'PSP, роутинг, методи, партнери, compliance — панель управління'          }, href: '/sandbox/payment-architecture/admin-panel',  phase: '4' },
            { label: { en: 'Chargeback Management',   ua: 'Chargeback Management'     }, sub: { en: 'Dispute lifecycle, evidence collection, chargeback rate control',    ua: 'Lifecycle диспуту, збір доказів, контроль рівня chargebacks'              }, href: '/sandbox/payment-architecture/chargeback',   phase: '4' },
            { label: { en: 'Personalization',     ua: 'Персоналізація'       }, sub: { en: 'BIN lookup, preferred method, GEO context, PSP success rate', ua: 'BIN lookup, кращий метод, GEO контекст, success rate PSP' }, href: '/sandbox/payment-architecture/personalization', phase: '5' },
            { label: { en: 'How to add a new PSP', ua: 'Як додати новий PSP' }, sub: { en: 'Checklist for integrating any new payment provider', ua: 'Чеклист для інтеграції нового платіжного провайдера' }, href: '/sandbox/payment-architecture/add-psp', phase: 'next' },
          ].map(doc => (
