          ...
          .map(([selector, theme]) =>
            colorConfig
              .map(([key, cfg]) => {
                ...
                return color ? `${selector === "" ? "" : `${selector} `}[data-chart=${id}] { --color-${key}: ${color}; }` : null
              })
              .filter(Boolean)
              .join("\n")
          )
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div key={item.dataKey}
              className={cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:size-2.5 [&>svg]:text-muted-foreground",
  ...
  return (
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
        return (
          <div key={item.value} className={cn("flex items-center gap-1.5 [&>svg]:size-3 [&>svg]:text-muted-foreground")}>
